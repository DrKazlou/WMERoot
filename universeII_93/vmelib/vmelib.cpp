...
int VMEBridge::checkIrqParamter(unsigned int level, unsigned int statusID)
{
    ...
    if ((statusID < 1) || (statusID > 256)) {
        *Err << "Parameter status/ID must be in 1..256!\n";
        return -2;
    }
...
//  setup IRQ handling
//----------------------------------------------------------------------------
int VMEBridge::setupIrq(int image, unsigned int irqLevel, unsigned int statusID,
...
int VMEBridge::waitIrq(unsigned int irqLevel, unsigned int statusID)
{
    ...
    ioctl(uni_handle, IOCTL_WAIT_IRQ, (irqLevel << 8) + statusID);
                       ...
                       int vas, int vdw, unsigned int bufNr)
{
    ...
    if (bufNr > dmaMaxBuf) {
        *Err << "DMAread: Buffer number too high! Only " << dmaMaxBuf + 1
             << " buffer(s) available!\n";
        return -2;
    }
