...
static const char Version[] = "0.93 (Jan 2006)";
...
module_param(vrai_bs, int, 0); // Default: Disabled!
...
static void __iomem *baseaddr = 0;     // Base addres of Tundra chip

static void __iomem *dmaBuf = 0;       // DMA buf addr in kernel space
...
#define remap_page_range(vma, from, phys_addr, size, prot)             \
({                                                                     \
    remap_pfn_range(vma, from, phys_addr >> PAGE_SHIFT, size, prot);   \
})
#endif
...
static void DMA_timeout(unsigned long ptr)
{
    ...
    printk("UniverseII: DMA timed out!\n");
    statistics.timeouts++;
}
...
static void MBX_timeout(unsigned long ptr)
{
    ...
    printk("UniverseII: MBX %ld timed out!\n", ptr);
...
static int universeII_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    if (minor < MAX_IMAGE)  // master image
    {
        ...
        retval = remap_page_range(vma, vma->vm_start, p->phys_start,
        ...
        if (minor == DMA_MINOR) // DMA
        {
            ...
            retval = remap_page_range(vma, vma->vm_start, dmaHandle,
                               vma->vm_end - vma->vm_start, vma->vm_page_prot);
            ...
            retval = remap_page_range(vma, vma->vm_start, p->buffer,
                               vma->vm_end - vma->vm_start, vma->vm_page_prot);
        }
    }
...
static int universeII_release(struct inode *inode, struct file *file)
{
    ...
    if (minor < MAX_IMAGE)
		release_mem_region(readl(baseaddr + aBS[minor]),
		             readl(baseaddr + aBD[minor]) - readl(baseaddr + aBS[minor]));
    ...
    switch (cmd) {
        ...
        case IOCTL_SET_IMAGE: {
                    ...
                    if ((image[i].opened == 2) && (i != minor)) {
                        ...
                        if (! ((iRegs.base <= imageStart)
                            ...
                            printk("imageStart1 = %x, imageEnd1 = %x, "
                                   ...
                                   imageStart, imageEnd);
                            return -1;  // overlap with existing image
                        }
                    }
                ...
                if (!iRegs.ms) {   // master image
                    ...
                    image[minor].phys_start = pciBase;
                }
                else {    // slave image
                ...
                if (iRegs.ms) { // slave image
                    ...
                    writel(image[minor].buffer - iRegs.base, baseaddr + aTO[minor]);
                }
                ...
                image[minor].vBase = ioremap(readl(baseaddr + aBS[minor]), iRegs.size);
        ...
        case IOCTL_SET_IRQ: {
                ...
                vstatid = isetup.vmeStatus - 1;

                if (irq_device[virq][vstatid].ok) {
                    printk ("UniverseII: irq/status combination is already in use !\n");
                    return -1;
                }
        ...
        case IOCTL_SET_MBX: {
                ...
                if (mbx_en & (0x10000 << arg)) {    // mbx already in use
                    spin_unlock(&mbx_lock);
                    return -1;
                }

                writel(mbx_en | (0x10000 << arg), baseaddr + LINT_EN);
        ...
        case IOCTL_RELEASE_MBX: {
                ...
                if ((lintEn & (0x10000 << arg)) == 0) {
                    spin_unlock(&mbx_lock);
                    return -1;
                }

                writel(lintEn & ~(0x10000 << arg), baseaddr + LINT_EN);
